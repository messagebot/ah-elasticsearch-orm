...
module.exports = function (api) {
  ...
  utils.prepareQuery = function (searchKeys, searchValues, start, end, dateField) {
    ...
    for (var i in searchKeys) {
      ...
      if (typeof (val) === 'string' && key.match(/^data\..*$/)) {
        val = val.toLowerCase()
      }
    ...
    api.cache.load(key, function (error, result) {
      ...
      utils.runWithLimit(client, method, args, function (error) {
        ...
        for (var i = 1; i < arguments.length; i++) {
          if (arguments[i]._shards && arguments[i]._shards.failures && arguments[i]._shards.failures.length > 0) {
            return callback(arguments[i]._shards.failures)
          }
