...
var scroll = function(api, alias, query, fields, callback){
  ...
  utils.runWithLimit(api, api.elasticsearch.client, 'search', [query], function getMoreUntilDone(error, data){
    ...
    if(data.hits.total !== results.length && data.hits.hits.length > 0){
      utils.runWithLimit(api, api.elasticsearch.client, 'scroll', [{
